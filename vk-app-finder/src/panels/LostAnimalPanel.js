...
import {
  CellButton, Div, Group, Header, PanelHeader, } from "@vkontakte/vkui";
...
class LostAnimalPanel extends React.Component {
  ...
  getShareText = () => {
    ...
    return (
      ...
      `Адрес: ${this.animal.address}. ${config.appUrl}#lost${this.animal.id}`
    );
  };
  ...
  render() {
    ...
    return (
      <>
        <PanelHeader left={<PanelHeaderBack onClick={() => this.props.goBack(!this.props.wasEverFetched)}/>}>
          Потерянный питомец
        </PanelHeader>
